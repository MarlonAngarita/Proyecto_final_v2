...
class RegistroUsuarioSerializer(serializers.ModelSerializer):
    ...
    id_rol = serializers.PrimaryKeyRelatedField(
        queryset=Roles.objects.all(), required=False, allow_null=True
    )
...
class UsuariosSerializer(serializers.ModelSerializer):
    ...
    tipo_documento_info = serializers.StringRelatedField(
        source="id_tipo_documento", read_only=True
    )
...
class CursosSerializer(serializers.ModelSerializer):
    profesor_info = serializers.StringRelatedField(
        source="id_profesor", read_only=True, allow_null=True
    )
...
class ForoSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {"id_usuario": {"queryset": Usuarios.objects.all()}}
...
class ModulosSerializer(serializers.ModelSerializer):
    class Meta:
        model = Modulos
        fields = "__all__"
        extra_kwargs = {"id_curso": {"queryset": Cursos.objects.all()}}
...
class QuizSerializer(serializers.ModelSerializer):
    class Meta:
        model = Quiz
        fields = "__all__"
        extra_kwargs = {"id_modulo": {"queryset": Modulos.objects.all()}}
...
class RachasUsuarioSerializer(serializers.ModelSerializer):
    ...
    def get_tiempo_desde_ultima_actividad(self, obj):
        ...
        if not obj.ultima_actividad:
            return {"dias": None, "horas": None, "mensaje": "Sin actividad previa"}
        ...
        try:
            ...
            return {"dias": dias, "horas": horas, "mensaje": mensaje}

        except Exception:
            return {"dias": None, "horas": None, "mensaje": "Error al calcular tiempo"}
    ...
    def validate(self, attrs):
        ...
        if "id_usuario" in attrs:
            ...
            existing_racha = (
                RachasUsuario.objects.filter(id_usuario=usuario)
                .exclude(id_racha=self.instance.id_racha if self.instance else None)
                .first()
            )
    ...
    def update(self, instance, validated_data):
        ...
        if (
            "ultima_actividad" not in validated_data
            and "dias_consecutivos" in validated_data
        ):
    ...
    def to_representation(self, instance):
        ...
        representation["metadata"] = {
            ...
            "requiere_atencion": self.get_estado_racha(instance)
            in ["en_peligro", "perdida"],
        }
