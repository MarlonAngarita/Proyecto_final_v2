...
export class Cursos implements OnInit {
      ...
      next: (cursos) => {
        ...
        if (cursos && cursos.length > 0) {
          ...
          this.cursosDisponibles = cursos.map((curso) => ({
            ...
            contenido:
              curso.contenido || `Contenido del curso ${curso.nombre_curso || curso.nombre}`,
  ...
  inscribirseCurso() {
    ...
    const yaInscrito = this.cursosInscritos.some(
      (curso) =>
        curso.id === this.cursoSeleccionado.id || curso.titulo === this.cursoSeleccionado.titulo,
    );
